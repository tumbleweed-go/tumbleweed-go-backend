...
router.get('/', async (req, res, next) => {
  ...
  let promise = new Promise((resolve, reject) => {
    ...
    fb.firestore.collection('users').get().then(snapshot => {
      let found = [];
  ...
  await promise.then(found => {
    ...
    logger.log('/user/get', 'Retrieved users.');
    res.status(200).send({ result: found });
  }).catch(() => {
    // Get tumbleweed fail.
    logger.log('/user/get', 'Error getting users.');
    res.status(200).send({ result: 'Error getting users.' });
  });
});
